...
const seed = async () => {
  ...
  let user = await User.findOne({ email: 'demo@birthday-board.dev' });
  if (!user) {
    user = await User.create({
      name: 'Demo User',
      email: 'demo@birthday-board.dev',
  ...
  const birthdays = [
    ...
    { name: 'Today Person 🎂', birthDate: new Date(today.getFullYear() - 28, today.getMonth(), today.getDate()), note: 'Birthday is today!' },
  ].map((b) => ({ ...b, createdBy: user!._id }));
